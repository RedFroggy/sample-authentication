...
public class CommandSet {
    ...
    /**
     * Exchange server and client public keys (in RSA mode)
     *
     * @param publicKey Client public key (to send to server)
     * @return Public keys exchange command
     */
    public static byte[] exchangePublicKeys(byte[] publicKey) {
        byte[] cmd = new byte[publicKey.length + 1];
        cmd[0] = Instruction.PUB.getCode();
        System.arraycopy(publicKey, 0, cmd, 1, publicKey.length);
        return cmd;
    }
}
